  ...
  if ($product && $quantity > 0) {
   ...
   // Send the user to the place order page if they click the Place Order button, also the cart should not be empty
    if (isset($_POST['placeorder']) && isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
      header('Location: plaaceorder.php');
      exit;
  }
...
if ($products_in_cart) {
    ...
    $stmt = $db->prepare('SELECT * FROM items WHERE id IN (' . $array_to_question_marks . ')');
                ...
                <tr>
                    <td class="img">
                        <a href="cart.php?page=product&id=<?=$product['id']?>">
                            <img src="dashoborad/uploaded/images//<?=$product['item_image']?>" width="50" height="50" alt="<?=$product['name']?>">
                        </a>
                    </td>
                    <td>
                        <a href="cart.php?page=product&id=<?=$product['id']?>"><?=$product['name']?></a>
                        <br>
                        <a href="cart.php?page=cart&remove=<?=$product['id']?>" class="remove btn btn-sm btn-danger">Remove</a>
                    </td>
